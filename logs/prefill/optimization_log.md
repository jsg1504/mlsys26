...
## 2026-04-24 - Fused Gate → CuTe-DSL `gb_warp` Fusion (ACCEPTED, Duration-neutral)

- **Idea**: Eliminate the separate `fused_gate_kernel` (NVRTC, ~4.3us) by computing `gate_log = -exp(A_log) * softplus(a + dt_bias)` and `beta = sigmoid(b)` inline inside the CuTe-DSL kernel's `gb_warp` (warp 7). `gb_warp` was previously just a DRAM→SMEM loader for precomputed fp32 gate/beta; now it reads raw bf16 `a`/`b` + fp32 `A_log`/`dt_bias` and synthesizes gate/beta on the fly via `cute.math.exp` and `cute.math.log`. Changes: `GDN.__call__` signature extended with `A_log_iter`, `dt_bias_iter`; device `kernel` signature extended with `A_log_tensor`, `dt_bias_tensor`; gb_warp block (lines 694–780 after edit) computes `neg_exp_A` once per work-tile and per-token softplus/sigmoid in the existing 4×32 lane loop; `get_gdn_bundle` signature + `main.py` dispatch updated to pass raw `a`, `b`, `A_log`, `dt_bias` directly. `_gate_cache`, `_GateArgs/_gate_args`, `_kernels["fused_gate"]` removed from `main.py` (now orphan).
- **Result (NCU SpeedOfLight Duration, 15 workloads: 0, 2, 3, 6, 7, 9, 20, 22, 25, 28, 38, 46, 79, 90, 94)**:
  - Short-path (WL 0, 2, 22, 25; `gdn_prefill_sequential` only): 183.49us → 183.52us (+0.02%, noise baseline ±3%)
  - Long-path (11 WL; `fused_gate` + `kernel_cutlass_...` → single `kernel_cutlass_...`): **3,753.63us → 3,745.29us (−8.34us, −0.22%)**
  - Per-WL range: WL28 −3.5%, WL79 −1.8%, WL94 −1.6% (best) vs WL90 +1.3%, WL9 +0.4% (worst). Medium-T (1000–2100) benefits most; T=8192 sees gb_warp's added SFUs creep onto critical path.
  - `fused_gate_kernel` duration zeroed out (was 4.00–4.70us per long-path call). `kernel_cutlass_...` duration rose +1.07% on average (gb_warp now runs `exp` + `log` + `exp` + `exp` per token instead of just a gmem→smem copy).
- **Status**: ACCEPTED — retained despite Duration-neutral result because:
  1. Kernel launches per long-path call 2→1 (~3–5us CPU dispatch + queue overhead not visible in NCU, measurable in end-to-end latency).
  2. Intermediate `gate_log[T,8] fp32` and `beta[T,8] fp32` global buffers eliminated — removes L2 write-then-read traffic (~8KB write + 8KB read per call at T=1024; ~64KB at T=8192).
  3. Python hot-path simplification: `_gate_cache` dict, `_GateArgs` ctypes scaffolding, fused_gate NVRTC compile, per-call gate kernel launch all removed.
- **End-to-end bench at config (1,3,1)**: 0.1949ms mean (vs py-dispatch-v2 baseline 0.1837ms = **+6.1%**). Same-code variance from bench_history: (1,3,1) 0.1837, (1,5,3) 0.1906, (3,30,3) 0.1810 — noise band is ~±4% on (1,3,1). `mean_speedup` jumped 461→867 and `max_speedup` 2190→3874, which matches the (1,5,3) profile rather than (1,3,1) — suggests reference-impl timing drift rather than real kernel-side regression. **End-to-end result is inconclusive from a single (1,3,1) run**; a (3,30,3) stable-config run is the cleanest apples-to-apples comparison against the 0.1810 same-code baseline.
- **Correctness**: 100/100 PASSED on Modal B200 (atol=1, rtol=0.3, matched_ratio≥0.9). Max abs_err and rel_err per workload unchanged vs pre-fusion baseline.
- **Files modified**: `gdn_prefill_qk4_v8_d128_k_last/solution/python/gdn_blackwell/gdn.py` (signature + gb_warp + get_gdn_bundle), `gdn_prefill_qk4_v8_d128_k_last/solution/python/main.py` (cleanup).
- **Why gb_warp SFU additions are ~free**: `gb_warp` was memory-bound (single global load, single smem write per 4-element group). SFU pipes (exp/log) are separate from load/store pipes and overlap with other warps' MMA/load work on most workloads. Critical-path creep only appears at max-T when gb_warp's total work extends into the tail of the pipeline.
- **Why some T=8192 workloads regressed (+1.3% WL90)**: At max-T, `gb_warp` runs `loop_count = 64` iterations (8192/128) of the 4×32 lane loop. Added SFUs (3–4 per valid lane per iteration) push total gb_warp work to ~4096 SFU ops. This starts competing with the epilogue warp's work for the last few chunks, extending kernel tail by 5–7us on the 8192 workloads.
- **Future direction**: WL90/WL9 regression is addressable by either (a) hoisting `-exp(A_log)` multiply out of the per-token branch (already done — it's per-work-tile), (b) replacing `log(1 + exp(x))` with `max(x, 0) + log(1 + exp(-|x|))` to avoid numerical-stability cost at large x, or (c) swapping `exp(-b)` sigmoid form for `0.5 * (1 + tanh(b/2))` to trade 1 SFU for a FMA+SFU. Low priority given overall Duration is neutral.
