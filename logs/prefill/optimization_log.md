...
## 2026-04-08 Iteration 47

- Idea: add a `long16`-only L2 persisting-cache access window on the reused `a` gate input, while leaving the recurrence math, `micro2` path, and row-tile dispatch unchanged.
- Why: the required Modal profile again stayed directionally consistent with the current notes despite the prefill harness caveat, with achieved occupancy only about `6.22%` on the smallest profiled shapes and `14.33%` to `18.85%` on the biggest ones, plus compute and memory throughput still below roughly `10%`. The refreshed local candidate ranking still listed a `long16` L2 persisting-cache window as the remaining current-family try-now experiment, and CUDA's access-policy-window guidance makes it the narrowest way to bias a repeatedly reused input region toward set-aside L2 without perturbing the proven long-path math.
- Result: the quick Modal benchmark passed all `100/100` workloads with `mean_speedup 265.90600`, `mean_latency_ms 0.95567`, `min_speedup 81.85`, and `max_speedup 859.05` versus the latest comparable logged quick baseline `mean_speedup 269.04501` and `mean_latency_ms 0.95700`.
- Decision: revert because correctness held but the weighted `mean_speedup` regressed versus the latest comparable quick baseline.

## 2026-04-08 Iteration 48

- Idea: add a `long16`-only L2 persisting-cache access window on the reused `b_gate` input, while leaving the recurrence math, `micro2` path, and row-tile dispatch unchanged.
- Why: the required Modal profile again stayed directionally consistent with the current notes despite the prefill harness caveat, with achieved occupancy only about `6.22%` on the smallest profiled shapes and `14.33%` to `18.85%` on the biggest ones, plus compute and memory throughput still below roughly `11%`. After the existing `a`-window revert in Iteration 47, the only remaining same-family single-buffer access-policy variant was the sibling `b_gate` stream, which still fits the persisting-window budget and is reread across the live `long16` row-tile CTAs without perturbing the proven math.
- Result: the quick Modal benchmark passed all `100/100` workloads with `mean_speedup 260.59130`, `mean_latency_ms 0.96247`, `min_speedup 73.37`, and `max_speedup 744.56` versus the latest comparable logged quick baseline `mean_speedup 269.04501` and `mean_latency_ms 0.95700`.
- Decision: revert because correctness held but the weighted `mean_speedup` regressed versus the latest comparable quick baseline.

## 2026-04-08 Iteration 49

- Idea: add a `long16`-only L2 persisting-cache access window on the reused `k` input, while capping the access-policy window to the device limit and tuning `hitRatio` to the persisting-L2 budget.
- Why: the required fresh Modal profile remained directionally consistent with the current prefill notes even though the profiling harness still lags the live wrapper dispatch: achieved occupancy stayed around `6.15%` to `18.85%`, compute and memory throughput stayed well below saturation, and the representative large-shape profile still showed only about `19%` to `20%` L2 hit rate. After the earlier `a` and `b_gate` persisting-window trials regressed, the remaining same-family single-buffer candidate with materially larger cross-CTA reuse was the shared `k` stream on the dominant live `long16` path. NVIDIA's CUDA Best Practices Guide also explicitly recommends tuning `num_bytes` and `hitRatio` together when the persistent region exceeds the L2 set-aside budget to reduce cache thrashing, which matches the much larger `k` tensor.
- Result: the quick Modal benchmark passed all `100/100` workloads with `mean_speedup 282.05599`, `mean_latency_ms 0.94532`, `min_speedup 81.61223`, and `max_speedup 821.72466` versus the latest comparable logged quick baseline `mean_speedup 269.04501` and `mean_latency_ms 0.95700`.
- Decision: commit because correctness held and the weighted `mean_speedup` improved versus the latest comparable quick baseline.

## 2026-04-08 Iteration 50

- Idea: extend the existing `k`-input L2 persisting-cache access window from the `long16` launch to the `micro2` launch as well, while leaving the recurrence math, row ownership, and higher-`N` dispatch unchanged.
- Why: research mode was `local_only`, so the ranking used only the live kernel, `logs/prefill/bench_history.jsonl`, `logs/prefill/optimization_log.md`, `docs/prefill_optimization_candidates_2026-04-08_en.md`, and the required fresh Modal profile. That profile stayed directionally consistent with the recent notes despite the prefill harness caveat: low-`N` shapes still launched only `64` to `128` blocks with achieved occupancy around `6.25%`, while the live wrapper still routes `61/100` contest workloads through `micro2`. Unlike the long path, `micro2` rereads the same `k` stream across `64` row tiles per `(seq, v_head)`, and those low-`N` `k` tensors fit comfortably inside the persisting-L2 set-aside budget, so extending the already-winning `k` window to the low-`N` path was the narrowest current-family experiment left that could reduce fixed memory-latency overhead without perturbing the proven math.
